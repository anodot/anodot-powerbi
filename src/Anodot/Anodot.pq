...
Anodot = [
	Authentication = [
	   Key = [KeyLabel = "Anodot API key", Label = "Anodot API key"]
	],
...
AnodotNavTable = (url as text, domain as text) as table =>
   ...
   GetMetricsNamesTable = (url as text) as table =>
	let
		...
		source = Web.Contents(url & "search/metrics/props", [ Headers = DefaultRequestHeaders, Content = postData, ManualCredentials = true ]),
		...
		navTable = Table.ToNavigationTable(result, {"MetricName"}, "MetricName", "Data", "ItemKind", "ItemName", "IsLeaf")
	in
	  navTable;
	...
	let
		postData = Json.FromValue([composite = []]), // TODO: add correct payload
		source = Web.Contents(url & "metrics/composite/execute", [ Headers = DefaultRequestHeaders, Query = [fromDate = "1649629416", toDate="1650129416"], Content = postData, ManualCredentials = true ]),
...
GetAlertsTable = (url as text, optional query) as table =>
	let
		source = Web.Contents(url & "alerts/triggered", [ Headers = DefaultRequestHeaders, Query = query, ManualCredentials = true ]),
...
GetAnomaliesTable = (url as text, optional query) as table =>
	let
		source = Web.Contents(url & "anomalies", [ Headers = DefaultRequestHeaders, Query = query, ManualCredentials = true ]),
		json = Json.Document(source)[anomalies],
		anomaliesTable = Table.FromRecords(json)        
	in
	   anomaliesTable;

shared Anodot.GetAnyAnomalies = (url) => (measureName as text, optional queryParams as record) => GetAno(measureName, url, queryParams);
   shared Anodot.GetNamedAnomalies = (measureName as text, url as text) => (optional queryParams as record) => GetAno(measureName, url, queryParams);
	...
	in
		...
		MakeExpression = (measureName as text) => 
			let
				...
				res = Text.Insert(txt, 17, "t" ) // As we can't setreserved "type" property
			in
				res;

Anodot.Icons = [
    Icon16 = { Extension.Contents("Anodot16.png"), Extension.Contents("Anodot20.png"), Extension.Contents("Anodot24.png"), Extension.Contents("Anodot32.png") },
    Icon32 = { Extension.Contents("Anodot32.png"), Extension.Contents("Anodot40.png"), Extension.Contents("Anodot48.png"), Extension.Contents("Anodot64.png") }
];
